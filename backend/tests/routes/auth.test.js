...
describe("POST /auth/register", () => {
    ...
    test("creates candidate_profile for job_seeker", async () => {
        ...
        const result = await pool.query("SELECT * FROM candidate_profiles WHERE user_id=(SELECT id FROM users WHERE email=$1)", ["john@test.com"])
        expect(result.rows.length).toBe(1)
    })

    test("creates company for recruiter", async () => {
        await registerUser({ email: "rec@test.com", role: "recruiter" })
        const result = await pool.query("SELECT * FROM companies WHERE user_id=(SELECT id FROM users WHERE email=$1)", ["rec@test.com"])
        expect(result.rows.length).toBe(1)
    })
})
...
describe("POST /auth/login", () => {
    ...
    test("stores hashed refresh token in DB", async () => {
        ...
        const result = await pool.query("SELECT * FROM refresh_tokens WHERE token=$1", [hash])
        expect(result.rows.length).toBe(1)
    })
    ...
    test("returns 400 if fields missing", async () => {
        const res = await request(app).post("/api/v1/auth/login").set("User-Agent", "Jest Test Runner").send({ email: "john@test.com" })
        expect(res.statusCode).toBe(400)
    })
})
...
describe("POST /auth/refresh", () => {
    ...
    test("returns new access token with valid refresh token", async () => {
        const res = await request(app).post("/api/v1/auth/refresh").set("User-Agent", "Jest Test Runner").send({ refresh_token: refreshToken })
        expect(res.statusCode).toBe(200)
        expect(res.body.access_token).toBeDefined()
    })

    test("returns 401 if no refresh token provided", async () => {
        const res = await request(app).post("/api/v1/auth/refresh").set("User-Agent", "Jest Test Runner").send({})
        expect(res.statusCode).toBe(401)
        expect(res.body.error).toBe("No refresh token provided")
    })

    test("returns 401 if refresh token is invalid", async () => {
        const res = await request(app).post("/api/v1/auth/refresh").set("User-Agent", "Jest Test Runner").send({ refresh_token: "invalidtoken" })
        expect(res.statusCode).toBe(401)
    ...
    test("returns 401 if refresh token is revoked", async () => {
        ...
        const res = await request(app).post("/api/v1/auth/refresh").set("User-Agent", "Jest Test Runner").send({ refresh_token: token })
...
describe("POST /auth/logout", () => {
    ...
    test("logs out and revokes refresh token", async () => {
        ...
        const result = await pool.query("SELECT revoked FROM refresh_tokens WHERE token=$1", [hash])
        expect(result.rows[0].revoked).toBe(true)
    })

    test("returns 401 if no access token provided", async () => {
        const res = await request(app).post("/api/v1/auth/logout").set("User-Agent", "Jest Test Runner").send({ refresh_token: refreshToken })
        expect(res.statusCode).toBe(401)
    })
